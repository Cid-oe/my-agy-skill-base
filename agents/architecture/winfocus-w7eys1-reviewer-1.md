---
name: winfocus-w7eys1-reviewer-1
description: 'Clean Phase 1 implementation of click-to-focus for Windows toast notifications. The diff aligns with ADR-001 and the design doc. The architecture is sound: P/Invoke type, two focused helper functions, activation event loop modeled on the proven `win-play.ps1` pattern, and proper PS 7+ delegation forwarding. The parentPid plumbing is correctly wired end-to-end (hook script, win-notify.ps1, WSL placeholder) for Phase 2 without adding unused logic now.'
kind: local
model: inherit
agy:
  version: 1.0.0
  ...
  - repo: PeonPing/peon-ping
    ...
    path: .gitban/agents/reviewer/inbox/WINFOCUS-w7eys1-reviewer-1.md

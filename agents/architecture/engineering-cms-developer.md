...
agy:
  ...
  - repo: VKirill/codex-starter-kit
    ...
    path: agents/engineering_cms_developer.toml
    format: toml
---
...
### MCP / Tool Routing
- Use Serena for CMS code navigation and targeted edits.
- Use Context7/official docs for current WordPress/Drupal/plugin APIs.
- Use GitNexus when CMS changes affect shared backend/frontend flows.
- Use database inspection only read-only and only when schema/content state is relevant.

### Skill Routing
- Prefer wordpress-developer, frontend-developer, backend/API patterns, security, testing-patterns, and Superpowers verification skills as relevant.
- Avoid unrelated mobile/ML/bot/Web3 skills unless explicitly needed.
</CODEx-TOOLING-SKILL-ROUTING>

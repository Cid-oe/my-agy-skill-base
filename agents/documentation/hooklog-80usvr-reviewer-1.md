---
name: hooklog-80usvr-reviewer-1
description: Card 80usvr adds a `--all` flag to `peon logs --session ID` so that sessions spanning midnight can be searched across all log files. The implementation covers peon.sh (Unix), install.ps1 (Windows parity), completions for bash and fish, documentation in README.md/README_zh.md/llms.txt, and comprehensive tests in both BATS and Pester.
kind: local
model: inherit
agy:
  version: 1.0.0
  ...
  - repo: PeonPing/peon-ping
    ...
    path: .claude/worktrees/agent-a9ddbd90/.gitban/agents/reviewer/inbox/HOOKLOG-80usvr-reviewer-1.md

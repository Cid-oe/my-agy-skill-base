---
name: m2close-ot0edu-reviewer-1
description: '**Tests:** "uses project name when no template configured" and "renders all 5 template variables correctly" both produce an empty `$notifyLog`, meaning the `Start-Process powershell.exe ... win-notify.ps1` child process did not finish writing `.notify-log.txt` before the test read it.'
kind: local
model: inherit
agy:
  version: 1.0.0
  ...
  - repo: PeonPing/peon-ping
    ...
    path: .gitban/agents/reviewer/inbox/M2CLOSE-ot0edu-reviewer-1.md
...
None. Both blockers are in-scope for this card.

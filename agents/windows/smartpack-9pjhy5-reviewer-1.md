---
name: smartpack-9pjhy5-reviewer-1
description: This merge commit integrates the Windows path_rules CLI parity work (bind/unbind/bindings subcommands in `install.ps1`) with the sprint branch's existing `Get-ActivePack` refactor. The conflict resolution correctly threads `$pathRulePack` fallback through every branch that previously called `Get-ActivePack` directly.
kind: local
model: inherit
agy:
  version: 1.0.0
  ...
  - repo: PeonPing/peon-ping
    ...
    path: .gitban/agents/reviewer/inbox/SMARTPACK-9pjhy5-reviewer-1.md

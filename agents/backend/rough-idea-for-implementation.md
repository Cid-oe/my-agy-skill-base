---
name: rough-idea-for-implementation
description: Add `roo` (Roo Code CLI) as a new backend provider in ralph-orchestrator, enabling `ralph run -b roo` and `ralph plan -b roo`.
kind: local
model: inherit
agy:
  version: 1.0.0
  ...
  - repo: mikeyobrien/ralph-orchestrator
    ...
    path: .agents/planning/2026-03-03-roo-cli-provider/rough-idea-for-implementation.md

---
name: smartpackdebt-inexon-reviewer-1
description: 'The entire runtime `path_rules` matching engine (the block that evaluates `$event.cwd` against `$config.path_rules` during event processing to select a pack) has been removed from the hook script. Previously this code lived between "Pick a sound" and the `pack_rotation` logic:'
kind: local
model: inherit
agy:
  version: 1.0.0
  category: backend
  tags:
  - SMARTPACKDEBT-inexon-reviewer-1
  compatibility:
  ...
  - repo: PeonPing/peon-ping
    ...
    path: .gitban/agents/reviewer/inbox/SMARTPACKDEBT-inexon-reviewer-1.md

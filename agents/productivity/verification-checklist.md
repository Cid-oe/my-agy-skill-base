...
description: Rules accumulate over time. Each workflow-changelog run MUST execute ALL rules at the specified depth. When a new type of drift is caught that an existing rule should have caught (but didn't exist or was too shallow), append a new rule here.
kind: local
model: inherit
agy:
  version: 1.0.0
  ...
  - repo: shanraisshan/claude-code-best-practice
    ...
    path: changelog/best-practice/claude-subagents/verification-checklist.md

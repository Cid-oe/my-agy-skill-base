...
agy:
  ...
  category: devops
  tags:
  - engineering_incident_response_commander
  compatibility:
  ...
  - repo: VKirill/codex-starter-kit
    ...
    path: agents/engineering_incident_response_commander.toml
    format: toml
---
...
Use `/codex:rescue` to delegate deep investigation when root-cause analysis stalls after 2-3 hypotheses. The Codex rescue subagent (GPT-5) takes an independent investigation pass with fresh context — useful when Claude is stuck in a wrong hypothesis loop.

Workflow:
1. Document what you've already tried and ruled out.
2. Invoke `/codex:rescue` with the symptom + your ruled-out hypotheses (so Codex doesn't repeat them).
3. Treat Codex's findings as another data point, not gospel — verify before acting on the recommended fix.

...
agy:
  ...
  category: ai
  tags:
  - engineering_email_intelligence_engineer
  compatibility:
  ...
  - repo: VKirill/codex-starter-kit
    ...
    path: agents/engineering_email_intelligence_engineer.toml
    format: toml
---
...
### MCP / Tool Routing
- Use local files/connectors containing the email thread first; do not infer missing thread content.
- Use Context7 or official docs when output schema or integration APIs are current/unstable.
- Use Serena/GitNexus only when implementing or tracing code that consumes extracted email data.

### Skill Routing
- Prefer email intelligence, data extraction, llm-app-patterns, error-handling, and Superpowers verification skills as relevant.
- Avoid UI/design/devops/database skills unless the email workflow explicitly requires them.
</CODEx-TOOLING-SKILL-ROUTING>

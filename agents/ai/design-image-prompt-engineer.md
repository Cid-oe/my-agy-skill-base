...
agy:
  ...
  - repo: VKirill/codex-starter-kit
    ...
    path: agents/design_image_prompt_engineer.toml
    format: toml
---
...
### MCP / Tool Routing
- Use project assets, existing design files, screenshots, and local UI code before external inspiration.
- Use Context7 or framework docs only when design guidance must map to implementation constraints.
- Use Vue/Nuxt/React docs MCP only when component APIs affect the design handoff.
- Use image generation only when the task asks for raster visuals or visual concepts.

### Skill Routing
- Prefer UI/design, inclusive visuals, copywriter, imagegen, accessibility, frontend handoff, and Superpowers brainstorming skills as relevant.
- Do not use backend/database/devops/security-heavy skills unless the design task explicitly depends on those constraints.
</CODEx-TOOLING-SKILL-ROUTING>

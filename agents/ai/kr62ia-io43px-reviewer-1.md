---
name: kr62ia-io43px-reviewer-1
description: 'This card hardens the Windows notification template engine by closing two gaps identified during the kr62ia review: (1) replacing a regex-only `task.error` test with a proper invoke-based test, and (2) aligning the `idle_prompt`/`elicitation_dialog` override guards with the Unix `peon.sh` structure.'
kind: local
model: inherit
agy:
  version: 1.0.0
  ...
  - repo: PeonPing/peon-ping
    ...
    path: .gitban/agents/reviewer/inbox/kr62ia-io43px-reviewer-1.md

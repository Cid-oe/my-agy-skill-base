---
name: anthropic-provider-proposal
description: 'This proposal outlines the implementation plan for native Anthropic Claude API support in Yao Agent. Currently, all LLM connectors use `type: "openai"`, and Anthropic detection relies on URL pattern matching, which is unreliable and architecturally incorrect.'
kind: local
model: inherit
agy:
  version: 1.0.0

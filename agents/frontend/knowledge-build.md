...
agy:
  ...
  - repo: Jennyee1/AcademicAgent
    ...
    path: .agents/workflows/knowledge-build.md
...
4. **运行知识盲区检测**:
   ```bash
   python skills/learning_path/scripts/analyze_knowledge.py --action detect_gaps --save
   ```

5. **生成学习路径**:
   ```bash
   python skills/learning_path/scripts/analyze_knowledge.py --action learning_path --save
   ```

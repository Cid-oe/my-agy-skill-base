...
agy:
  ...
  - repo: VKirill/codex-starter-kit
    ...
    path: agents/testing_evidence_collector.toml
    format: toml
---
...
### MCP / Tool Routing
- Use Playwright/browser tooling first for visual evidence, interaction checks, screenshots, traces, and reproducibility.
- Use Serena only for targeted code navigation when visual evidence points to a specific implementation area.
- Use GitNexus when a suspected defect crosses routes, shared flows, API contracts, or package boundaries.
- Use Context7 or Vue/Nuxt/Nuxt UI MCP only for current framework behavior needed to interpret evidence.
- Use Postgres MCP only when observed behavior depends on real database state.

### Skill Routing
- Prefer playwright-skill, testing-patterns, goal-achievement-review, bug-hunter, find-bugs, code-review-checklist, gitnexus-debugging, gitnexus-impact-analysis, and Superpowers verification/debugging skills as relevant.
- Do not use implementation/design/backend/data/devops/ML-heavy skills unless the QA task explicitly expands into that domain.
</CODEx-TOOLING-SKILL-ROUTING>
